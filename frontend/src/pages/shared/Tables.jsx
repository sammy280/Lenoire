  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Table Management">
        {canManage && (
          <button
            onClick={() => { setShowAdd(true); setForm({ name: '', seatCount: 4 }); }}
      ...
      <div className="flex gap-2 flex-wrap text-xs sm:text-sm">
        ...
        <span className="px-3 py-1.5 bg-orange-500/10 text-orange-400 rounded-xl font-semibold">💳 {stats.waiting} Waiting</span>
        <span className="px-3 py-1.5 bg-accent text-muted-foreground rounded-xl font-semibold">{tables.length} Total</span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {tables.map(table => (
          <div
            key={table.id}
            className={cn(
              'rounded-2xl border-2 p-3 sm:p-5 space-y-2 sm:space-y-3 transition-all relative',
              tableColors[table.status] || 'border-border'
            )}
          >
            {/* Edit / Delete */}
            {canManage && (
              <div className="absolute top-2 right-2 flex gap-1">
                <button
                  ...
                  className="p-1 sm:p-1.5 bg-accent/80 hover:bg-accent rounded-lg text-muted-foreground hover:text-foreground transition-colors"
                >
                  <Edit2 className="w-3 h-3" />
                </button>
                <button
                  onClick={() => handleDelete(table)}
                  className="p-1 sm:p-1.5 bg-red-500/10 hover:bg-red-500/20 rounded-lg text-red-400 transition-colors"
                >
            ...
            <div className="flex flex-wrap gap-1">
              {table.seats?.map(seat => (
                <div
                  key={seat.id}
                  className={cn(
                    'w-8 h-8 sm:w-10 sm:h-10 rounded-lg border-2 text-[10px] sm:text-xs font-bold flex items-center justify-center relative group',
                    seat.isOccupied
                ...
                >
                  ...
                  {canManage && !seat.isOccupied && (
                    <button
                      onClick={() => removeSeat.mutate({ tableId: table.id, seatId: seat.id })}
              ...
              {canManage && (
                <button
                  onClick={() => addSeat.mutate(table.id)}
                  className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg border-2 border-dashed border-border flex items-center justify-center text-muted-foreground hover:border-primary hover:text-primary transition-colors"
                >
            ...
            <p className="text-xs text-muted-foreground">
              {table.seats?.filter(s => s.isOccupied).length}/{table.seats?.length} occupied
            </p>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-sm">
            <h3 className="font-semibold mb-4">
              {editTable ? `Edit Table: ${editTable.name}` : 'Add New Table'}
            </h3>
            <form
              onSubmit={e => {
                e.preventDefault();
            ...
            >
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => setForm(f => ({ ...f, seatCount: Math.max(1, f.seatCount - 1) }))}
                  ...
                  <button
                    type="button"
                    onClick={() => setForm(f => ({ ...f, seatCount: f.seatCount + 1 }))}
                    className="p-2 bg-accent rounded-lg hover:bg-accent/80"
                  >
